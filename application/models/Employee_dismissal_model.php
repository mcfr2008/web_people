...
class Employee_dismissal_model extends CI_Model {
	...
	public function get_dismissal_byid_model() 
    {
        ...
        $this->db->select('
			employee_dismissal.*,
		');
		$this->db->from('employee_dismissal');
		$this->db->where('employee_dismissal.fld_id',$_data['fld_id']); 
		$data['data'] = $this->db->get()->result_array()[0];

		return $data;
    }

	public function post_dismissal_model() 
    {
		...
		$this->db->set('fld_employee_id',$_data['info']['fld_employee_id']);
		$this->db->set('fld_organization_id',$_data['info']['fld_organization_id']);
		$this->db->set('fld_docno',$data_docno);
		$this->db->set('fld_date',$_data['detail']['fld_date']);
		$this->db->set('fld_other_details',$_data['detail']['fld_other_details']);
		
		$this->db->set('fld_trash',1);
		$this->db->set('fld_created_id',$_SESSION['people_id']);
		$this->db->set('fld_created_at',date('Y-m-d H:i:s'));
		$this->db->insert('employee_dismissal');
		...
		//-------------------------------------Activity_log-------------------------------------------------
			...
			$data_activity_log = array(
				"fld_activity" => 'เพิ่ม' ,
				"fld_table_name" => 'employee_dismissal',
				"fld_table_id" => $id,
	...
	public function put_dismissal_model() 
    {
		...
		$this->db->update('employee_dismissal');
