...
class Employee_divisions_model extends CI_Model {
	...
	public function get_divisions_active_status_model()
	{
	...
	public function get_divisions_all_model() 
    {
        ...
        $this->db->select('*');
		$this->db->from('employee_divisions');
	...
	public function get_divisions_model()
	{
		...
		$search_department = $_data['search_department'];

		$this->db->select("
			employee_divisions.* , 
			CASE
				WHEN employee_divisions.fld_active = 1  THEN '<span class=\"text-primary\"><i class=\"fa fa-eye\"></i>  ใช้งาน</span>'
				WHEN employee_divisions.fld_active = 0  THEN '<span class=\"text-black\"><i class=\"fa fa-eye-slash\"></i> ไม่ใช้งาน</span>'
				ELSE ''
		...
		if($search_text != ''){
			$this->db->group_start();
				for($i=0;$i<$count_search_text;$i++){
					$this->db->like('employee_divisions.fld_id',$search_text[$i]);
				}
				for($i=0;$i<$count_search_text;$i++){
					$this->db->or_like('employee_divisions.fld_name_th',$search_text[$i]);
				}
				for($i=0;$i<$count_search_text;$i++){
					$this->db->or_like('employee_divisions.fld_name_en',$search_text[$i]);
				}
			$this->db->group_end();
		}
		if($search_active != ''){
			$this->db->where('employee_divisions.fld_active',$search_active);
		}
		if($search_department != ''){
			$this->db->where('employee_divisions.fld_department_id',$search_department);
		}
		$this->db->where('employee_divisions.fld_trash',1);
		$this->db->limit($PerPage,$pageNumber);
		$this->db->order_by('employee_divisions.fld_order','ASC'); 
		$data['data'] = $this->db->get()->result_array();
	...
	public function get_divisions_byid_model() 
    {
		...
		');
		$this->db->from('employee_divisions');
		$this->db->where('employee_divisions.fld_id',$_data['fld_id']); 
		$data['data'] = $this->db->get()->result_array()[0];

		return $data;
    }

	public function post_divisions_model() 
    {
		...
		//-------------------------------------Activity_log-------------------------------------------------
			...
			$data_activity_log = array(
				"fld_activity" => 'เพิ่ม' ,
				"fld_table_name" => 'employee_divisions',
				"fld_table_id" => $id,
	...
	public function put_divisions_model() 
    {
		...
		$this->db->update('employee_divisions');
	...
	public function put_divisions_active_model() 
	{
	...
	public function delete_divisions_model() 
    {
