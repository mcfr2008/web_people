...
<script>
	...
	function search_function_1() 
	{
		...
		get_positions(search_text,search_active);
	};
	...
	{ 
		...
		$('#btn_add').click(function () {
			...
			$('#positions_title').text('เพิ่ม');
			$('#positions_type').val('add');
		});

		$('#data_positions').on('click','.btn_edit',function(){ 
			
			clear_data_manage();
			$('#manageModal').modal('show');
			$('#positions_title').text('แก้ไข');
			$('#positions_type').val('edit');

			var id = $(this).closest('tr').find('.id').text();
			var order = $(this).closest('tr').find('.order').text();

			$('#positions_order_old').val(order);
			$('#positions_order_new').val(order);
			
			get_positions_byid(id,'manage');
		});
		...
		$('#data_positions').on('click','.btn_del',function(){ 
		...
		$('#data_positions').on('click','.btn_active_1',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					put_positions_active(id,active);
				}
			})

		});

		$('#data_positions').on('click','.btn_active_0',function(){ 
	...
	{ 
		...
		$('#positions_order_old').val('');
		$('#positions_order_new').val('').removeClass( "is-warning is-valid" );
	...
	function validate_manage() 
	{
		...
		if($("#positions_order_new").val() == '' ){
			$( "#positions_order_new" ).removeClass( "is-valid" ).addClass( "is-warning" );
			validate = 'false';
		}else if($('#positions_order_new').val() != '' ){
			$( "#positions_order_new" ).removeClass( "is-warning" ).addClass( "is-valid" );
		}
	...
	function get_positions(search_text,search_active) 
	{ 
		...
		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_positions/get_positions')?>",
		data : {
	...
	function get_positions_active_status() 
	{ 

		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_positions/get_positions_active_status')?>",
		dataType: "JSON",
		success: function (data) {
	...
	function get_positions_byid(id,type) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_positions/get_positions_byid')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	{ 
		...
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_positions/delete_positions')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function positions_manage() 
	{ 
	
		if($('#positions_type').val() == 'add'){
			var url = '<?= site_url('employee_positions/post_positions')?>';
		}else if($('#positions_type').val() == 'edit'){
			var url = '<?= site_url('employee_positions/put_positions')?>';
		}
		...
		$.ajax({
			...
			dataType:'JSON',
			data:{
				fld_id : $('#positions_id').val(),
				fld_name_th : $('#name_th').val(),
				fld_name_en : $('#name_en').val(),
				fld_group : $('#group').val(),
				fld_other_details : $('#other_details').val(),
				order_old : $('#positions_order_old').val(),
				order_new : $('#positions_order_new').val()
			},
		}).done(function(data){
	...
	function put_positions_active(id,active) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_positions/put_positions_active')?>",
			dataType:'JSON',
			data:{
				fld_id : id,
				fld_active : active,
