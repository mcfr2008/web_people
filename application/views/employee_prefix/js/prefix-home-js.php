...
<script>
	...
	function search_function_1() 
	{
		...
		get_prefix(search_text,search_active);
	};
	...
	{ 
		...
		$('#btn_add').click(function () {
			...
			$('#prefix_title').text('เพิ่ม');
			$('#prefix_type').val('add');
		});

		$('#data_prefix').on('click','.btn_edit',function(){ 
			
			clear_data_manage();
			$('#manageModal').modal('show');
			$('#prefix_title').text('แก้ไข');
			$('#prefix_type').val('edit');

			var id = $(this).closest('tr').find('.id').text();
			
			get_prefix_byid(id,'manage');
		});
		...
		$('#data_prefix').on('click','.btn_del',function(){ 
		...
		$('#data_prefix').on('click','.btn_active_1',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					put_prefix_active(id,active);
				}
			})

		});

		$('#data_prefix').on('click','.btn_active_0',function(){ 
	...
	function get_prefix(search_text,search_active) 
	{ 
		...
		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_prefix/get_prefix')?>",
		data : {
	...
	function get_prefix_active_status() 
	{ 

		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_prefix/get_prefix_active_status')?>",
		dataType: "JSON",
		success: function (data) {
	...
	function get_prefix_byid(id,type) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_prefix/get_prefix_byid')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	{ 
		...
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_prefix/delete_prefix')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function prefix_manage() 
	{ 
	
		if($('#prefix_type').val() == 'add'){
			var url = '<?= site_url('employee_prefix/post_prefix')?>';
		}else if($('#prefix_type').val() == 'edit'){
			var url = '<?= site_url('employee_prefix/put_prefix')?>';
		}
	...
	function put_prefix_active(id,active) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_prefix/put_prefix_active')?>",
			dataType:'JSON',
			data:{
				fld_id : id,
				fld_active : active,
