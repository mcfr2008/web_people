...
<script>
	...
	function search_function_1() 
	{
		...
		get_religions(search_text,search_active);
	};
	...
	{ 
		...
		$('#btn_add').click(function () {
			...
			$('#religions_title').text('เพิ่ม');
			$('#religions_type').val('add');
		});

		$('#data_religions').on('click','.btn_edit',function(){ 
			
			clear_data_manage();
			$('#manageModal').modal('show');
			$('#religions_title').text('แก้ไข');
			$('#religions_type').val('edit');

			var id = $(this).closest('tr').find('.id').text();
			
			get_religions_byid(id,'manage');
		});
		...
		$('#data_religions').on('click','.btn_del',function(){ 
		...
		$('#data_religions').on('click','.btn_active_1',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					put_religions_active(id,active);
				}
			})

		});

		$('#data_religions').on('click','.btn_active_0',function(){ 
	...
	function get_religions(search_text,search_active) 
	{ 
		...
		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_religions/get_religions')?>",
		data : {
	...
	function get_religions_active_status() 
	{ 

		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_religions/get_religions_active_status')?>",
		dataType: "JSON",
		success: function (data) {
	...
	function get_religions_byid(id,type) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_religions/get_religions_byid')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function delete_religions(id) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_religions/delete_religions')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function religions_manage() 
	{ 
	
		if($('#religions_type').val() == 'add'){
			var url = '<?= site_url('employee_religions/post_religions')?>';
		}else if($('#religions_type').val() == 'edit'){
			var url = '<?= site_url('employee_religions/put_religions')?>';
		}
	...
	function put_religions_active(id,active) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_religions/put_religions_active')?>",
			dataType:'JSON',
			data:{
				fld_id : id,
				fld_active : active,
