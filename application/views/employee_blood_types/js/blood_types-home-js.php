...
<script>
	...
	function search_function_1() 
	{
		...
		get_blood_types(search_text,search_active);
	};
	...
	{ 
		...
		$('#btn_add').click(function () {
			...
			$('#blood_types_title').text('เพิ่ม');
			$('#blood_types_type').val('add');
		});

		$('#data_blood_types').on('click','.btn_edit',function(){ 
			
			clear_data_manage();
			$('#manageModal').modal('show');
			$('#blood_types_title').text('แก้ไข');
			$('#blood_types_type').val('edit');

			var id = $(this).closest('tr').find('.id').text();
			
			get_blood_types_byid(id,'manage');
		});
		...
		$('#data_blood_types').on('click','.btn_del',function(){ 
		...
		$('#data_blood_types').on('click','.btn_active_1',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					put_blood_types_active(id,active);
				}
			})

		});

		$('#data_blood_types').on('click','.btn_active_0',function(){ 
	...
	function get_blood_types(search_text,search_active) 
	{ 
		...
		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_blood_types/get_blood_types')?>",
		data : {
	...
	function get_blood_types_active_status() 
	{ 

		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_blood_types/get_blood_types_active_status')?>",
		dataType: "JSON",
		success: function (data) {
	...
	function get_blood_types_byid(id,type) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_blood_types/get_blood_types_byid')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function delete_blood_types(id) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_blood_types/delete_blood_types')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function blood_types_manage() 
	{ 
	
		if($('#blood_types_type').val() == 'add'){
			var url = '<?= site_url('employee_blood_types/post_blood_types')?>';
		}else if($('#blood_types_type').val() == 'edit'){
			var url = '<?= site_url('employee_blood_types/put_blood_types')?>';
		}
	...
	function put_blood_types_active(id,active) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_blood_types/put_blood_types_active')?>",
			dataType:'JSON',
			data:{
				fld_id : id,
				fld_active : active,
