...
<script>
	...
	function search_function_1() 
	{
		...
		get_genders(search_text,search_active);
	};
	...
	{ 
		...
		$('#btn_add').click(function () {
			...
			$('#genders_title').text('เพิ่ม');
			$('#genders_type').val('add');
		});

		$('#data_genders').on('click','.btn_edit',function(){ 
			
			clear_data_manage();
			$('#manageModal').modal('show');
			$('#genders_title').text('แก้ไข');
			$('#genders_type').val('edit');

			var id = $(this).closest('tr').find('.id').text();
			
			get_genders_byid(id,'manage');
		});
		...
		$('#data_genders').on('click','.btn_del',function(){ 
		...
		$('#data_genders').on('click','.btn_active_1',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					put_genders_active(id,active);
				}
			})

		});

		$('#data_genders').on('click','.btn_active_0',function(){ 
	...
	function get_genders(search_text,search_active) 
	{ 
		...
		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_genders/get_genders')?>",
		data : {
	...
	function get_genders_active_status() 
	{ 

		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_genders/get_genders_active_status')?>",
		dataType: "JSON",
		success: function (data) {
	...
	function get_genders_byid(id,type) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_genders/get_genders_byid')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function delete_genders(id) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_genders/delete_genders')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function genders_manage() 
	{ 
	
		if($('#genders_type').val() == 'add'){
			var url = '<?= site_url('employee_genders/post_genders')?>';
		}else if($('#genders_type').val() == 'edit'){
			var url = '<?= site_url('employee_genders/put_genders')?>';
		}
	...
	function put_genders_active(id,active) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_genders/put_genders_active')?>",
			dataType:'JSON',
			data:{
				fld_id : id,
				fld_active : active,
