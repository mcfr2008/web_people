...
<script>
    ...
    $(document).ready(function () {
		...
		get_vaccine_covid_19_type_active_status();
	...
	function search_function_1() 
	{
		...
		get_vaccine_covid_19_type(search_text,search_active);
	};
	...
	{ 
		...
		$('#btn_add').click(function () {
			...
			$('#vaccine_covid_19_type_title').text('เพิ่ม');
			$('#vaccine_covid_19_type_type').val('add');
		});

		$('#data_vaccine_covid_19_type').on('click','.btn_edit',function(){ 
			
			clear_data_manage();
			$('#manageModal').modal('show');
			$('#vaccine_covid_19_type_title').text('แก้ไข');
			$('#vaccine_covid_19_type_type').val('edit');

			var id = $(this).closest('tr').find('.id').text();
			
			get_vaccine_covid_19_type_byid(id,'manage');
		});
		...
		$('#data_vaccine_covid_19_type').on('click','.btn_del',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					delete_vaccine_covid_19_type(id);
		...
		$('#data_vaccine_covid_19_type').on('click','.btn_active_1',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					put_vaccine_covid_19_type_active(id,active);
				}
			})

		});

		$('#data_vaccine_covid_19_type').on('click','.btn_active_0',function(){ 
	...
	{ 
		$('#vaccine_covid_19_type_type').val('');
		$('#vaccine_covid_19_type_id').val('');
	...
	function get_vaccine_covid_19_type(search_text,search_active) 
	{ 
		...
		$.ajax({
		type: "POST",
		url: "<?= site_url('Vaccine_covid_19_type/get_vaccine_covid_19_type')?>",
		data : {
		...
		success: function (data) {
			...
			$('#data_vaccine_covid_19_type').empty();
			$.each(data['data'], function (id, value) { 
				...
				$('#data_vaccine_covid_19_type').append(
				'<tr>'+
	...
	function get_vaccine_covid_19_type_active_status() 
	{ 

		$.ajax({
		type: "POST",
		url: "<?= site_url('Vaccine_covid_19_type/get_vaccine_covid_19_type_active_status')?>",
		dataType: "JSON",
		success: function (data) {
	...
	function get_vaccine_covid_19_type_byid(id,type) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('Vaccine_covid_19_type/get_vaccine_covid_19_type_byid')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
			...
			if(type == 'manage'){
				...
				$('#vaccine_covid_19_type_id').val(id);
	...
	function delete_vaccine_covid_19_type(id) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('Vaccine_covid_19_type/delete_vaccine_covid_19_type')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function vaccine_covid_19_type_manage() 
	{ 
	
		if($('#vaccine_covid_19_type_type').val() == 'add'){
			var url = '<?= site_url('Vaccine_covid_19_type/post_vaccine_covid_19_type')?>';
		}else if($('#vaccine_covid_19_type_type').val() == 'edit'){
			var url = '<?= site_url('Vaccine_covid_19_type/put_vaccine_covid_19_type')?>';
		}
		...
		$.ajax({
			...
			dataType:'JSON',
			data:{
				fld_id : $('#vaccine_covid_19_type_id').val(),
	...
	function put_vaccine_covid_19_type_active(id,active) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('Vaccine_covid_19_type/put_vaccine_covid_19_type_active')?>",
			dataType:'JSON',
			data:{
				fld_id : id,
				fld_active : active,
