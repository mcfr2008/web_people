...
<script>
    ...
    $(document).ready(function () {
		...
		get_disability_type_active_status();
	...
	function search_function_1() 
	{
		...
		get_disability_type(search_text,search_active);
	};
	...
	{ 
		...
		$('#btn_add').click(function () {
			...
			$('#disability_type_title').text('เพิ่ม');
			$('#disability_type_type').val('add');
		});

		$('#data_disability_type').on('click','.btn_edit',function(){ 
			
			clear_data_manage();
			$('#manageModal').modal('show');
			$('#disability_type_title').text('แก้ไข');
			$('#disability_type_type').val('edit');

			var id = $(this).closest('tr').find('.id').text();
			
			get_disability_type_byid(id,'manage');
		});
		...
		$('#data_disability_type').on('click','.btn_del',function(){ 
		...
		$('#data_disability_type').on('click','.btn_active_1',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					put_disability_type_active(id,active);
				}
			})

		});

		$('#data_disability_type').on('click','.btn_active_0',function(){ 
	...
	{ 
		$('#disability_type_type').val('');
		$('#disability_type_id').val('');
	...
	function get_disability_type(search_text,search_active) 
	{ 
		...
		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_disability_type/get_disability_type')?>",
		data : {
	...
	function get_disability_type_active_status() 
	{ 

		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_disability_type/get_disability_type_active_status')?>",
		dataType: "JSON",
		success: function (data) {
	...
	function get_disability_type_byid(id,type) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_disability_type/get_disability_type_byid')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function delete_disability_type(id) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_disability_type/delete_disability_type')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function disability_type_manage() 
	{ 
	
		if($('#disability_type_type').val() == 'add'){
			var url = '<?= site_url('employee_disability_type/post_disability_type')?>';
		}else if($('#disability_type_type').val() == 'edit'){
			var url = '<?= site_url('employee_disability_type/put_disability_type')?>';
		}
	...
	function put_disability_type_active(id,active) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_disability_type/put_disability_type_active')?>",
			dataType:'JSON',
			data:{
				fld_id : id,
				fld_active : active,
