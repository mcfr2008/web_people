...
<script>
	...
	function search_function_1() 
	{
		...
		get_status(search_text,search_active);
	};
	...
	{ 
		...
		$('#btn_add').click(function () {
			...
			$('#status_title').text('เพิ่ม');
			$('#status_type').val('add');
		});

		$('#data_status').on('click','.btn_edit',function(){ 
			
			clear_data_manage();
			$('#manageModal').modal('show');
			$('#status_title').text('แก้ไข');
			$('#status_type').val('edit');

			var id = $(this).closest('tr').find('.id').text();
			
			get_status_byid(id,'manage');
		});
		...
		$('#data_status').on('click','.btn_del',function(){ 
		...
		$('#data_status').on('click','.btn_active_1',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					put_status_active(id,active);
				}
			})

		});

		$('#data_status').on('click','.btn_active_0',function(){ 
	...
	function get_status(search_text,search_active) 
	{ 
		...
		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_status/get_status')?>",
		data : {
	...
	function get_status_active_status() 
	{ 

		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_status/get_status_active_status')?>",
		dataType: "JSON",
		success: function (data) {
	...
	function get_status_byid(id,type) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_status/get_status_byid')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	{ 
		...
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_status/delete_status')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	{ 
		...
		if($('#status_type').val() == 'add'){
			var url = '<?= site_url('employee_status/post_status')?>';
		}else if($('#status_type').val() == 'edit'){
			var url = '<?= site_url('employee_status/put_status')?>';
		}
	...
	function put_status_active(id,active) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_status/put_status_active')?>",
			dataType:'JSON',
			data:{
				fld_id : id,
				fld_active : active,
