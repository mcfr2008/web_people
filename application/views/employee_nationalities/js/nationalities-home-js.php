...
<script>
    ...
    $(document).ready(function () {
		...
		get_nationalities_active_status();
	...
	function search_function_1() 
	{
		...
		get_nationalities(search_text,search_active);
	};
	...
	{ 
		...
		$('#btn_add').click(function () {
			...
			$('#nationalities_title').text('เพิ่ม');
			$('#nationalities_type').val('add');
		});

		$('#data_nationalities').on('click','.btn_edit',function(){ 
			
			clear_data_manage();
			$('#manageModal').modal('show');
			$('#nationalities_title').text('แก้ไข');
			$('#nationalities_type').val('edit');

			var id = $(this).closest('tr').find('.id').text();
			
			get_nationalities_byid(id,'manage');
		});
		...
		$('#data_nationalities').on('click','.btn_del',function(){ 
		...
		$('#data_nationalities').on('click','.btn_active_1',function(){ 
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					put_nationalities_active(id,active);
				}
			})

		});

		$('#data_nationalities').on('click','.btn_active_0',function(){ 
	...
	{ 
		$('#nationalities_type').val('');
		$('#nationalities_id').val('');
	...
	function get_nationalities(search_text,search_active) 
	{ 
		...
		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_nationalities/get_nationalities')?>",
		data : {
	...
	function get_nationalities_active_status() 
	{ 

		$.ajax({
		type: "POST",
		url: "<?= site_url('employee_nationalities/get_nationalities_active_status')?>",
		dataType: "JSON",
		success: function (data) {
	...
	function get_nationalities_byid(id,type) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_nationalities/get_nationalities_byid')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	{ 
		...
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_nationalities/delete_nationalities')?>",
			dataType:'JSON',
			data:{
				fld_id : id
			},
	...
	function nationalities_manage() 
	{ 
	
		if($('#nationalities_type').val() == 'add'){
			var url = '<?= site_url('employee_nationalities/post_nationalities')?>';
		}else if($('#nationalities_type').val() == 'edit'){
			var url = '<?= site_url('employee_nationalities/put_nationalities')?>';
		}
	...
	function put_nationalities_active(id,active) 
	{ 
	
		$.ajax({
			type:'POST',
			url: "<?= site_url('employee_nationalities/put_nationalities_active')?>",
			dataType:'JSON',
			data:{
				fld_id : id,
				fld_active : active,
